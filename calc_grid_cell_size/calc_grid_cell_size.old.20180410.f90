program calc_grid_cell_size
  ...
  character (len=255) :: file_in, file_ot, var_in_dummy
  ...
  if (argc /= 3) then
    write(*,'(a62,I4,a9)') 'STOP: Bad number of arguments (INFILE, INVAR and OUTFILE needed) but ', argc, ' provided'
    WRITE(*,'(A14)') '  NOTHING DONE'
    stop
  end if
  call get_command_argument(1, file_in)
  call get_command_argument(2, var_in_dummy)
  call get_command_argument(3, file_ot)
  ...
  ! GET VAR IDs
  nf_stat = NF90_INQ_VARID(ncid_in, var_in_dummy, id_v_dummy)
  call nf90_check_error(nf_stat, 'error inq varid '//var_in_dummy)
  ...
  allocate(val_v_lon(n_lon), val_v_lat(n_lat), val_v_time(n_time), &
           val_v_depth(n_depth), val_v_dummy(n_lon, n_lat, n_depth, n_time))
  ...
  nf_stat = NF90_GET_VAR(ncid_in, id_v_dummy, val_v_dummy)
  call nf90_check_error(nf_stat, 'error get var '//var_in_dummy)
  
  
  ! READ _FillValue OR missing_value OF VARIABLE 'h'
  fillval_dummy_bad = .FALSE.
  nf_stat = nf90_get_att(ncid_in, id_v_dummy, '_FillValue', fillval_dummy)
  
  if ( nf_stat .eq. -43 ) then
    
    write(*,*) 'Warning: attribute _FillValue not found for variable '//&
                var_in_dummy//'. Trying missing_value.'
    nf_stat = nf90_get_att(ncid_in, id_v_dummy, 'missing_value', fillval_dummy)
    if ( nf_stat .eq. -43 ) then
      write(*,*) 'Warning: attribute missing_value not found for variable '//&
                 var_in_dummy//'. Using no _FillValue'
      fillval_dummy_bad = .TRUE.
    else
      call nf90_check_error(nf_stat, 'error get att missing_value of var '//var_in_dummy)
    end if
    
  else
    
    call nf90_check_error(nf_stat, 'error get att _FillValue of var '//var_in_dummy)
  ...
  if (fillval_dummy_bad) then
    ...
    write(*,*) '  input variable: '//trim(var_in_dummy)
  ...
  nf_stat = nf90_def_var(ncid_ot, 'cell_area', NF90_DOUBLE, &
                         (/id_d_lon,id_d_lat/), &
                         id_v_cell_area)
  call nf90_check_error(nf_stat, 'error def var cell_area')
  
  
  ! create dummy_depth variable and deflate
  if (.not. fillval_dummy_bad) then
    nf_stat = nf90_def_var(ncid_ot, 'mask', NF90_BYTE, &
  ...
  nf_stat = nf90_set_atts_cell_area(ncid_ot, 'cell_area',  &
                                         fillval_cell_area)
  call nf90_check_error(nf_stat, 'error write atts for cell_area')
  
  if (.not. fillval_dummy_bad) then
    nf_stat = nf90_set_atts_mask(ncid_ot, 'mask', fillval_mask)
  ...
  nf_stat = nf90_set_global_atts(ncid_in, ncid_ot, &
                                 ...
                                 trim(file_in)//' '//trim(var_in_dummy)//' '//&
                                 trim(file_ot))
  call nf90_check_error(nf_stat, 'error copy atts global')
  ...
  if (fillval_dummy_bad) then
    ...
    call calc_mask(val_v_dummy, val_v_mask,            &
  ...
  deallocate(val_v_lon, val_v_lat, val_v_time, val_v_depth, & 
             val_v_mask, val_v_cell_area, val_v_dummy)
  ...
  contains
    ...
    function nf90_set_atts_cell_area(ncid, varname, fillval)
      ...
      nf_stat = nf90_put_att(ncid, id_v, 'description', 'Cell_areaq is the horizontal area of a gridcell.')
...
!$omp end parallel do
    ...
    subroutine calc_mask(dummy, mask, count, fillval_dummy) 
      ...
      ! of DEPTH at a specific horizontal location and time as soon as we 
      ! encounter a missing value. 
!$omp parallel do private(tmp_depth, tmp_h, iLa, iLo, iTi, proceed_depth, iZ) shared(mask) num_threads( 4 )
      do iTi = 1, nTi
        do iLa = 1, nLa
          do iLo = 1, nLo
